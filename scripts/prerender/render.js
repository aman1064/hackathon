...
const crawl = require("./src/puppeteer-utils.js").crawl;
const runServer = require("./src/run-server.js");
...
const run = async (userOptions, { fs } = { fs: nativeFs }) => {
  ...
  const server = options.externalServer ? null : await runServer(options);
  ...
  await crawl({
    ...
    afterFetch: async ({ page, route, browser, addToQueue }) => {
      ...
      if (options.sanitizePage) await options.sanitizePage({ page });
      ...
      if (options.saveAs === "html") {
        ...
        const newRoute = await page.evaluate(() => location.toString());
