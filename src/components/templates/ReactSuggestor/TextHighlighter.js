...
export default class TextHighlighter extends React.Component {
  ...
  _renderHighlighted(str, key) {
    ...
    return React.createElement(
      this.props.highlightWrapElement,
      props,
      children
    );
  }
  ...
  _highlight(text, search) {
    ...
    if (!this.props.isGlobalSearch) {
      ...
      if (!matchedArr) {
        ...
        const begin = matchedArr.index,
          end = matchedArr.index + matchedArr[0].length;

        const normal = text.slice(0, begin),
          highlight = text.slice(begin, end);
        if (normal) {
          resultHTML.push(this._renderNormal(normal, key));
          key++;
        }
        if (highlight) {
          resultHTML.push(this._renderHighlighted(highlight, key));
          key++;
        }
  ...
  render() {
    ...
    if (this.props.search instanceof Array) {
      const finalStr = this.props.search
        .map(search => this._escapeString(search))
        .join("|");
    ...
    if (!this._isChildrenPrimitive(this.props.children)) {
      if (React.Children.count(this.props.children) > 1) {
        console.error(
          "TextHighlighter must contain one wrapper element as a child"
        );
        return;
      ...
      const parsedChildren = this._parseAndHighlight(
        parentNode.props.children,
        sRegex
      );
      /**Clone the parent element */
      const parsedParentNode = React.cloneElement(
        parentNode,
        parentNode.props,
        parsedChildren
      );
      return <React.Fragment>{parsedParentNode}</React.Fragment>;
    }

    return (
      <span className="thWrapper">
