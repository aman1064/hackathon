...
class CrossButton extends React.Component {
  ...
  render() {
    let cross;
    if (
      this.props.renderCross &&
      this.props.renderCross.constructor.name === "Function"
    ) {
    ...
    return (
      <span
        ref={this.setRef}
        className="sugCrossBt"
        onClick={this.props.onClick}
        style={{ display: this.props.inputVal ? "inline-block" : "none" }}
      >
        {cross}
      </span>
    );
  ...
  render() {
    const input = this.props.renderInput(
      {
        ...
        placeholder: this.props.inputProps.placeholder.placeholder,
    ...
    return (
      <div className="sugInputWrapper">
        {input}
        {this.props.shouldRenderCross && (
          <CrossButton
            crossRef={this.props.crossRef}
            onClick={this.props.onClick}
            renderCross={this.props.renderCross}
  ...
  render() {
    return (
      <div className="sugMenuWrapper" id="sugMenu" ref={this.setRef}>
        {this.props.separateOptionsLabel && !this.props.typedValue && <label>Top {this.props.separateOptionsLabel}</label>}
        {this.props.children}
        {this.props.allowCreate && this.props.value && <button type="submit" onClick={this.props.children.onMouseEnter} className="textButton viewedJobsCTA sugItemCustomWrapper">Create {this.props.value}</button>}
      </div>
    );
  }
}
...
export default class Suggestor extends React.Component {
  ...
  inputRef(node) {
    ...
    if (
      "getInputRef" in this.props &&
      this.props.getInputRef.constructor === Function
    ) {
  ...
  isMenuOpen() {
    ...
    return this.state.isMenuOpen && (this.props.dataItems.length >0 || this.props.allowCreate);
  }
  ...
  openMenu() {
    ...
    if (
      !this.isMenuOpen() &&
      (limit >= this.props.searchKeysLimit || this.props.shouldListOpenOnFocus)
    ) {
  ...
  styleCross() {
    const inputNode = this.refs.input,
      inputNodeBoundaries = inputNode.getBoundingClientRect(),
      crossBound = this.refs.crossBt
        ? this.refs.crossBt.getBoundingClientRect()
        : {};
    if (this.props.shouldRenderCross && this.refs.crossBt) {
      this.refs.crossBt.style.top =
        inputNodeBoundaries.height / 2 - crossBound.height / 2 + "px";
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const {
      ...
      isTextArea,
      dataItems,
      shouldOpenWithoutSearch
    } = this.props;
    /**Style menu on each render */
    if (
      (this.isMenuOpen() && !prevState.isMenuOpen) ||
      (dataItems.length && !prevProps.dataItems.length)
    ) {
      this.styleMenu();
    ...
    if (
      activeElementIndex &&
      activeElementIndex >= this.getDataItems().length
    ) {
  ...
  handleChangeUtil(event, isMultiSearch) {
    ...
    if (!this.props.allowCreate && (!searchWord || searchWord.length < this.props.searchKeysLimit)) {
      this.getMenuData("");
      this.initialiseState();
      return;
    }
    this.getMenuData(searchWord);
    if (!this.isMenuOpen()) {
      this.setState({
        isMenuOpen: true
      });
  ...
  handleUpDownKey(keyName, addVal) {
    ...
    let newActiveIndex =
      (activeElementIndex + addVal + (addVal < 0 ? items.length : 0)) %
      items.length;
    let i = 0;
    while (
      items[newActiveIndex].disabled ||
      String(items[newActiveIndex].labelType).toLowerCase() === "s"
    ) {
      i++;
      newActiveIndex =
        (newActiveIndex + addVal + (addVal < 0 ? items.length : 0)) %
        items.length;
      if (i === items.length) {
        break;
      }
    ...
    if (
      ...
      newActiveIndex !== activeElementIndex
    ) {
   ...
   */
  getDataItems() {
    const limit = Math.min(this.props.dataLimit, this.suggestorData.length);
  ...
  renderMenu() {
    ...
    const items = this.getDataItems();
    // if (items.length === 0 && items[0].id === "notFound") {
    //   menuItems = this.props.notFoundTemplate(items[0], searchStr);
    // } else {
      menuItems = items && items.map((item, index) => {
        let itemElem = "";
        if ("getListItemTemplate" in this.props) {
          itemElem = this.props.getListItemTemplate(
            item,
        ...
        if (itemElem && !React.isValidElement(itemElem)) {
          console.error(
            "Returned element is not a valid react element in getListItemTemplate function"
          );
          return undefined;
        }
        let className =
          index === this.state.activeElementIndex ? "sugHighlighted" : "";
        ...
        return (
          <React.Fragment>
          {index === this.props.separateOptionsThreshold && !this.props.typedValue && <label>Other {this.props.separateOptionsLabel} </label>}
          <MenuItem
    ...
    const menuItemsWrap = (
      ...
      <div
        onMouseEnter={this.ignoreBlurEvent}
        onMouseLeave={this.unSetActiveELement}
      >
        {menuItems}
      </div>
    );
    ...
    return <Menu menuRef={this.menuRef} allowCreate={this.props.allowCreate} {...this.props} >{menu}</Menu>;
  }
  render() {
    const { renderInput, value, shouldRenderCross, dataItems } = this.props;
    ...
    inputProps = {
      ...
      handleChange: this.props.isMultiSearch
        ? this.handleChangeMultiSearch
...
Suggestor.defaultProps = {
  ...
  isTextArea: false,
  usePlaceholderPropsAsInputPlaceholder: false,
  renderInput(props, isTextArea) {
    ...
    const {
      placeholder,
      usePlaceholderPropsAsInputPlaceholder,
      labelClassName,
      ...rest
    } = props;
    return (
      <BaseInput
        autoComplete="off"
        label={usePlaceholderPropsAsInputPlaceholder ? undefined : placeholder}
        placeholder={
          usePlaceholderPropsAsInputPlaceholder ? placeholder : undefined
        }
        labelClassName={labelClassName && labelClassName.labelClass ?labelClassName.labelClass : undefined } 
        {...rest}
      />
    );
  },
...
Suggestor.propTypes = {
  ...
  usePlaceholderPropsAsLabel: PropTypes.bool
};
